function changeColor() {
    let id = document.getElementById("diagramEventsMsg").value;
    let newcolor = document.getElementById("newcolor").value;
    let text = JSON.parse(myDiagram.model.toJson());
    $.each(text.nodeDataArray, function (index, value) {
        if (this.key == id) {
            this.brush = newcolor
        }
    });
    document.getElementById("mySavedModel").value = JSON.stringify(text);
    load();
}
function init() {
    ...
    myDiagram.addDiagramListener("ObjectSingleClicked",
        e => {
            ...
            if (!(part instanceof go.Link)) showMessage(part.data.key);
        });
...
function addNodeAndLink(e, obj) {
    ...
    var newdata = { text: "text", brush: olddata.brush, dir: olddata.dir, parent: olddata.key };
...
function showMessage(s) {
    document.getElementById("diagramEventsMsg").value = s;
}
...
function save() {
    ...
    if (name == "") {
        alert("Trường TÊN rỗng");
    }
    else {
        let text = myDiagram.model.toJson();
        let id = document.getElementById("IDMindMap").value;
        let img = myDiagram.makeImage({
            size: new go.Size(200, 170)
        }).outerHTML;
        var arrImg = img.match(/.{1,500}/g)
        document.getElementById("mySavedModel").value = myDiagram.model.toJson();
        myDiagram.isModified = false;
        myDiagram.model = go.Model.fromJson(document.getElementById("mySavedModel").value);
        load();
        $.ajax({
            url: "/GoJSMindMap/Create",
            data: { id: id, name: name, text: text, listUser: "", ListDep: "" },
            dataType: "text",
            type: "POST",
            success: function (response) {
                if (id == 0) {
                    window.location = "https://localhost:44324/GoJSMindMap/";
                    save1(response, 0, arrImg, arrImg.length);
                }
                else {
                    save1(id, 0, arrImg, arrImg.length);
                }
            }
        });
    }
}
function save1(id, item, arrImg, leng) {
    if (item < leng) {
        $.ajax({
            url: "/GoJSMindMap/SaveImg",
            data: { id: id, arrImg: arrImg[item], index: item },
            dataType: "text",
            type: "POST",
            success: function (response) {
                item++;
                save1(id, item, arrImg, leng);
            }
        });
    }
}
